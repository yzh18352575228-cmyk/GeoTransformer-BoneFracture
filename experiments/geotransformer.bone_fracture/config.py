...
# data
_C.data = edict()
_C.data.dataset_root = osp.join(_C.root_dir, 'my_data_processed')

# pretrained weights (set to None to train from scratch)
_C.pretrained = osp.join(_C.root_dir, 'weights', 'geotransformer-3dmatch.pth.tar')
...
_C.train.augmentation_noise = 0.0005       # 0.5mm noise for bone scale
_C.train.augmentation_rotation = 1.0
_C.train.rotation_magnitude = 10.0          # 10° displacement (tighter → more fracture overlap)
_C.train.translation_magnitude = 0.008      # 8mm displacement (tighter)
...
_C.test.rotation_magnitude = 10.0
_C.test.translation_magnitude = 0.008

# evaluation -- thresholds set to clinical requirements (2mm, 5deg)
_C.eval = edict()
_C.eval.acceptance_overlap = 0.0
_C.eval.acceptance_radius = 0.005          # 5mm inlier acceptance
_C.eval.inlier_ratio_threshold = 0.05
_C.eval.rmse_threshold = 0.002             # 2mm -- clinical success bar
_C.eval.rre_threshold = 5.0                # 5 degrees -- clinical success bar
_C.eval.rte_threshold = 0.002              # 2mm -- clinical success bar

# ransac
_C.ransac = edict()
_C.ransac.distance_threshold = 0.005       # 5mm RANSAC threshold
...
_C.optim = edict()
_C.optim.lr = 1e-4
_C.optim.lr_decay = 0.97                    # slower decay for more epochs
_C.optim.lr_decay_steps = 1
_C.optim.weight_decay = 1e-6
_C.optim.max_epoch = 120                    # 120 epochs with pretrained weights
_C.optim.grad_acc_steps = 1

# backbone - ADAPTED for bone scale (~1m object, 10mm voxel)
_C.backbone = edict()
_C.backbone.num_stages = 4
_C.backbone.init_voxel_size = 0.01         # 10mm voxels (vs 25mm for 3DMatch)
_C.backbone.kernel_size = 15
_C.backbone.base_radius = 2.5
_C.backbone.base_sigma = 2.0
_C.backbone.init_radius = _C.backbone.base_radius * _C.backbone.init_voxel_size   # 0.0075
_C.backbone.init_sigma = _C.backbone.base_sigma * _C.backbone.init_voxel_size      # 0.006
...
# model - Global
_C.model = edict()
_C.model.ground_truth_matching_radius = 0.008     # 8mm -- relaxed for better coarse matching training signal
_C.model.num_points_in_patch = 32       # reduced from 64 for small bone fragments
...
_C.fine_matching.acceptance_radius = 0.005       # 5mm -- fine matching inlier radius
...
# loss - Fine level
_C.fine_loss = edict()
_C.fine_loss.positive_radius = 0.005              # 5mm -- fine matching positive radius
